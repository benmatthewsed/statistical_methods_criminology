...
Later on we'll consider how we can bring our understanding of the large world - for example, how an incident becomes a crime - to bear during statistical modelling. In this session we'll overview the most common way in which criminologists understand the 'small world' of their spreadsheets: the general linear model.
...
The coefficients from Poisson models (which range from $-\infty$ to $\infty$) are converted to be predicted counts that are non-negative integers via the $\log$ link function. This lets us have coefficients which are unbounded, but then convert these to non-negative integers as our count outcome demands.
