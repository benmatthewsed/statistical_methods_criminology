...
In simple terms this technique uses the regression model to simulate a bunch of coefficients values from the regression model's variance-covariance matrix and and then calculate the VD for each one these simulated coefficient values. Simulating draws from the variance-covariance matrix gives a set of draws which together reflect the uncertainty in each of the regression coefficients (as expressed in their standard errors) and the correlation between these coefficients (as expressed in the covariance between the coefficients).
